...
class RoleRepository(IRepository):
    ...
    @connection
    async def create(self, data: Dict, session: AsyncSession) -> Role:
        existing_role = await session.execute(
            select(Role).filter(Role.name == data.model_dump()["name"])
        )
        if existing_role:
    ...
    @connection
    async def get_by_id(self, role_id: int, session: AsyncSession = None) -> Role:
        result = await session.execute(select(Role).filter(Role.id == role_id))
        return result.scalars().first()

    @connection
    async def update(
        self, role_id: int, data: Dict, session: AsyncSession = None
    ) -> Role:
    ...
    @connection
    async def delete(self, role_id: int, session: AsyncSession = None) -> bool:
